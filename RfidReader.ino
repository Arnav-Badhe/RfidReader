...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode == 200 && payload.substring(0, i1) == "0") {
      ...
      char buff[20];
      payload.substring(i2 + 1, i3).toCharArray(buff, payload.substring(i2 + 1, i3).length());

      lcd.setCursor(offLen(buff), 0);
      lcd.print(payload.substring(i2 + 1, i3));

      payload.substring(i3 + 1, i4).toCharArray(buff, payload.substring(i3 + 1, i4).length());

      lcd.setCursor(offLen(buff), 1);
      lcd.print(payload.substring(i3 + 1, i4));

      payload.substring(i4 + 1, i5).toCharArray(buff, payload.substring(i4 + 1, i5).length());

      lcd.setCursor(offLen(buff), 2);
      lcd.print(payload.substring(i4 + 1, i5));
